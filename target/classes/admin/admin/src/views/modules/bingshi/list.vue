        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '病名' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.bingshiName" placeholder="病名" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('bingshi','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/yiyuanziyuanguanli/upload/bingshiMuBan.xls"
                        >批量导入病史数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('bingshi','导入导出')"
                                style="display: inline-block"
                                action="yiyuanziyuanguanli/file/upload"
                                :on-success="bingshiUploadSuccess"
                                :on-error="bingshiUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('bingshi','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "bingshi.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="病因">
                        <template slot-scope="scope">
                            {{scope.row.bingshiBingyin}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="bingshiXianzhuang"
                                   header-align="center"
                                   label="现状">
                        <template slot-scope="scope">
                            {{scope.row.bingshiXianzhuang}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="bingshiText"
                                      ...
                                      label="药单">
                        <template slot-scope="scope">
                            <span v-if="scope.row.bingshiText != null &&scope.row.bingshiText.length>10">
                                {{scope.row.bingshiText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.bingshiText}}
                            </span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="禁忌">
                        <template slot-scope="scope">
                            {{scope.row.bingshiJinji}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="bingshiZhuyishixiang"
                                   header-align="center"
                                   label="注意事项">
                        <template slot-scope="scope">
                            {{scope.row.bingshiZhuyishixiang}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('bingshi','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('bingshi','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('bingshi','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                ...
                bingshiName : null,
                bingshiBingyin : null,
                bingshiXianzhuang : null,
                bingshiText : null,
                bingshiJinji : null,
                bingshiZhuyishixiang : null,
                createTime : null,
            },
            dataList: [],
            pageIndex: 1,
            ...
            json_fields: {
                ...
                //本表字段
                     '病名': "bingshiName",
                     '病因': "bingshiBingyin",
                     '现状': "bingshiXianzhuang",
                     '药单': "bingshiText",
                     '禁忌': "bingshiJinji",
                     '注意事项': "bingshiZhuyishixiang",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :"bingshi",//当前表表名,
                        sumColum : 'bingshi_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'bingshi_name',//分组字符串字段
                        // types : 'bingshi_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.bingshiName!= '' && this.searchForm.bingshiName!= undefined) {
                    params['bingshiName'] = '%' + this.searchForm.bingshiName + '%'
                }
                                                
                params['bingshiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            bingshiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "bingshi/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
